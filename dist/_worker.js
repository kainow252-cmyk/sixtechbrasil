...
${a[a.length-1].response.slice(0,800)}`,p=await Ge(c,d,t.env.AI);a.push(p),i=p.response}const s=a.filter(c=>c.source==="cloudflare").length,l=a.filter(c=>c.source==="internal").length;return t.json({task:e,steps:a.length,cloudflareSteps:s,internalSteps:l,results:a,final:i})});E.post("/api/document/generate",async t=>{if(!await Ne(t))return t.json({error:"Não autorizado"},401);const{agentId:r,docType:o,instructions:a,context:i}=await t.req.json(),s=J.find(p=>p.id===r);if(!s)return t.json({error:"Agente não encontrado"},404);const c=[{role:"system",content:`${s.system}
...
Instruções: ${a}`}],d=await t.env.AI.run(s.model,{messages:c,max_tokens:4096,stream:!0});return new Response(d,{headers:{"Content-Type":"text/event-stream; charset=utf-8","Cache-Control":"no-cache",Connection:"keep-alive","Access-Control-Allow-Origin":"*"}})});E.post("/api/document/analyze",async t=>{if(!await Ne(t))return t.json({error:"Não autorizado"},401);const{agentId:r,fileContent:o,fileName:a,instruction:i}=await t.req.json(),s=J.find(p=>p.id===r);if(!s)return t.json({error:"Agente não encontrado"},404);const l=`${s.system}
...
${i||"Analise este documento e aponte melhorias, erros e ajustes necessários."}`,d=await t.env.AI.run(s.model,{messages:[{role:"system",content:l},{role:"user",content:c}],max_tokens:4096,stream:!0});return new Response(d,{headers:{"Content-Type":"text/event-stream; charset=utf-8","Cache-Control":"no-cache",Connection:"keep-alive","Access-Control-Allow-Origin":"*"}})});E.post("/api/chat",async t=>{const{messages:e,model:r}=await t.req.json(),o=r||h.balanced;e.some(i=>i.role==="system")||e.unshift({role:"system",content:`Você é o assistente inteligente da SixTech Brasil — plataforma multiagente de IA.
Seja útil, preciso e responda em português brasileiro por padrão.
Se o usuário falar inglês, responda em inglês.`});const a=await t.env.AI.run(o,{messages:e,max_tokens:2048,stream:!0});return new Response(a,{headers:{"Content-Type":"text/event-stream; charset=utf-8","Cache-Control":"no-cache",Connection:"keep-alive","Access-Control-Allow-Origin":"*"}})});E.get("/api/models",t=>t.json({models:Object.entries(h).map(([e,r])=>({key:e,id:r,label:{fast:"⚡ Llama 3.2 3B — Rápido",balanced:"⚖️ Llama 3.1 8B — Balanceado",powerful:"💪 Llama 3.3 70B — Poderoso",coder:"💻 Qwen2.5 Coder 32B — Código",reason:"🧠 DeepSeek R1 32B — Raciocínio",kimi:"🎯 Kimi K2.6 1T — Orquestrador",gpt:"🤖 GPT-OSS 120B — Avançado",gemma:"💎 Gemma 3 12B — Google"}[e]||e}))}));E.get("/api/status",t=>t.json({status:"online",version:"3.0.0",platform:"SixTech MAS — Multi-Agent System",repos:{"sixtech-workspace":{agents:5,type:"Python FastAPI + Ollama",url:Z},sixtechworkspace:{type:"Cloudflare Workers AI + SSE",url:fr},"kndev-IA":{type:"OpenHands + opencode (RAR)",note:"Integrado ao developer agent"},sixtechbrasil:{type:"CF Pages — plataforma principal",url:"https://sixtechbrasil.pages.dev"}},agents:J.length,models:Object.keys(h).length,features:["hybrid routing","SSE streaming","smart orchestration","pipeline mode","fallback chain"],timestamp:new Date().toISOString()}));E.get("/",t=>t.html(`<!DOCTYPE html>
<html lang="pt-BR">
<head>
<meta charset="UTF-8">
...
</html>`));const rt=new Ct,kr=Object.assign({"/src/index.tsx":E});let St=!1;for(const[,t]of Object.entries(kr))t&&(rt.route("/",t),rt.notFound(t.notFoundHandler),St=!0);if(!St)throw new Error("Can't import modules from ['/src/index.tsx','/app/server.ts']");export{rt as default};
